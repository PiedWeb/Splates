...
```php
use PiedWeb\Splates\{Engine, Template\Theme};
...
The following looks ups, _regardless of where they are called from_, would resolve to the following files:
...
The fallback functionality is a bit different however since with folders, it's _opt in_, you need to prefix the template name with the folder name. With themes, all template names implicitly will be resolved and fallback according to the hierarchy setup.

## Additional Customization

This functionality is powered by the `PiedWeb\Splates\Template\ResolveTemplatePath` interface. If you'd prefer a more complex or specific path resolution, you can just implement your own and assign it to the engine instance with:
